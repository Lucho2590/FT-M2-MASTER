...
document.querySelector('#createdBy').innerHTML += ' Luciano' // Agrega el nombre del crear de la app con una etiqueta con ID. El += concatena un String. 
...
ToDo.prototype.completeToDo = function() {
    // Esta funcion cambia el estado de complete a true.
...
function buildToDo(todo, index) {
    // Tu código acá:
    let toDoShell = document.createElement('div'); // Crea un nuevo 'div' en el HTML
    toDoShell.className = 'toDoShell'; // Le agisna una clase a ese nuevo 'div'creado.

    let toDoText = document.createElement('span'); //Crea un 'span'. 
    toDoText.innerHTML = todo.description // le agrega un texto al 'span', que se le pasa por parametro:- builToDo(TODO, index)
    toDoText.id = index; // Se asigna un ID que se pasa por parametro. 

    if (todo.complete === true) {
        toDoText.className = 'completeText' // al tener todo completo, le modifica la clase al objeto creado. 
    };
    toDoShell.appendChild(toDoText); // introduce el 'span'dentro del 'div'ya creado.

    return toDoShell; // deveuelve la 'div'(como si fuere una caja) con el 'span'dentro.
}
...
function buildToDos(toDos) {
    // Tu código acá:
    var array = toDos.map(buildToDo); // MAP recorre el array pasado por parametro (toDos), utilizando la calback para insertarte los elementos HTML.
...
function displayToDos() {
    // Tu código acá:
    var toDoContainer = document.getElementById('toDoContainer'); // Selecciona un elemento del HTML que tiene declarado el id toDoContainer y lo guarda en una variable.
    toDoContainer.innerHTML = ''; // le agrega a ese elemento seleccionado un string vacio.
    var array = buildToDos(toDoItems);
    array.forEach(function(element) {
        toDoContainer.appendChild(element)
    }); // agarra cada posicion de buildToDos y los agrega al div con id toDoContainer.
}
